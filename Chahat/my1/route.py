...
from forms import RegistrationForm, LoginForm, UpdateAccountForm, SearchBar, HobbiesForm
...
def home():
    formq=SearchBar()
    return render_template('home.html', form=LoginForm(), forml=RegistrationForm(), formq=formq)
...
def about():
    ...
    if form_d.validate_on_submit():
        ...
        return redirect(url_for('home'))
    return render_template('about.html', form=LoginForm(), forml=RegistrationForm(), form_d=form_d)
...
def account():
    ...
    return render_template('account.html', curr_user=curr_user,selected_prod=selected_prod)
...
def search_result():
    ...
    if formq.validate_on_submit():
        ...
        if bool(text == 'All'):
            ...
            print(query_str)
        return render_template('search_result.html', form=LoginForm(), forml=RegistrationForm(), formq=formq, hob=results)
    return redirect(url_for('home'))


@app.route("/add_product/<int:prod_id>")
def add_product(prod_id):
    print('reacgh add products')
    if current_user.is_authenticated:
...
def product(prod_id,bol_):
    prod = Hobies.query.get_or_404(prod_id)
    bol=bol_
    return render_template('product.html', prod = prod, form=LoginForm(), forml=RegistrationForm(), bol = bol)
